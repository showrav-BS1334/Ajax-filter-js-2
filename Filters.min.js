      ...
      anchorCustomClick: function (t) {
        return m(this)
          ...
          .on("click", function (e) {
            ...
            $("#btnfilterconfirm").trigger("click");
          });
      },
    }),
          ...
          ("#/noFilterApplied" !== n &&
            (e && t && k() && m.address.history(!0),
              ...
              m
                ...
                .done(function (e) {
                  let eWindow = window, aInner = 'inner';
                  if (!('innerWidth' in window)) {
                    aInner = 'client';
                    eWindow = document.documentElement || document.body;
                  }
                  var result = { width: eWindow[aInner + 'Width'], height: eWindow[aInner + 'Height'] };
                  if (result.width < 1025) {
                    let ifAdmin = $('.admin-header-links');
                    let headerHeight = $(".responsive-nav-wrapper-parent")[0].scrollHeight;
                    if (ifAdmin.length > 0)
                      headerHeight = headerHeight + 35;
                    window.scrollTo({ top: $(".product-selectors").offset().top - headerHeight, behavior: "smooth" });
                  }
                  else
                   window.scrollTo({ top: $(".product-selectors").offset().top - 70, behavior: "smooth" });
                  ...
                  if (F.isProductRefreshTriggeredByScroll) {
                    let resultScriptDom = !document.querySelector(".viewmode-icon.list.selected") ?
                      m(e).filter("div.product-grid.nop7SpikesAjaxFiltersGrid")[0].querySelector("#product-price-script")
                      : m(e).filter("div.product-list.nop7SpikesAjaxFiltersGrid")[0].querySelector("#product-price-script");
                    let productScriptDom = document.createElement("script");
                    productScriptDom.innerText = resultScriptDom.innerText;
                    document.querySelector(".nop7SpikesAjaxFiltersGrid").appendChild(productScriptDom);
                  } else {
                    window.scrollTo({ top: $(".product-selectors").offset().top - 60, behavior: "smooth" });
                  }
                  // intelisale custom code
                  ...
                  if (("" !== n && (F.changeViewModeOrderByAndPageSize(e), F.setPriceRangeValues(e)), F.refreshProducts(e, F.isProductRefreshTriggeredByScroll, c, s))) {
                    ...
                    var counter = $('ul.selected-options-list li span').length;
                    $('span.filter-counter').html('(' + counter + ')');
      ...
      m(document).ready(function () {
        ...
        $("#btnfilterconfirm").click(function () {
          $(".specificationFilterPanel7Spikes")
            .not(".has-specification-slider")
            .find('.attribute-options-select :selected, .checkbox-list input[type="checkbox"]:checked')
            .each(function () {
              var e = $(this),
          ...
          $(".manufacturerFilterPanel7Spikes").find('.attribute-options-select :selected, .checkbox-list input[type="checkbox"]:checked').each(function () {
            var i = $(this),
          ...
          $(".vendorFilterPanel7Spikes").find('.attribute-options-select :selected, .checkbox-list input[type="checkbox"]:checked').each(function () {
            var i = $(this),
              r = i.closest(".filtersGroupPanel");
            let obj = { r, i };
            vendorfilter.push(obj);
          })

          for (let j = 0; j < specfilter.length; j++) {
            e.requestProductsForSelectedFilters(specfilter[j].i, specfilter[j].e);
          }
          for (let k = 0; k < menufilter.length; k++) {
            e.requestProductsForSelectedFilters(menufilter[k].r, menufilter[k].i);
          }
          for (let q = 0; q < vendorfilter.length; q++) {
            e.requestProductsForSelectedFilters(vendorfilter[q].r, vendorfilter[q].i);
          }
          e.requestProductsForSelectedFilters(m(".priceRangeFilterPanel7Spikes"));
        });
        ...
        var t,
          r = m(".nopAjaxFilters7Spikes .filter-buttons a.clearFilterOptionsAll");
        function i() {
          m(window)
